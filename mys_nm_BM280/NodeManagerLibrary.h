...
#endif
// OpenHABSensorConfiguration default child_id
#ifndef OpenHABSensorConfiguration_CHILD_ID
  #define OPENHABCONFIGURATION_CHILD_ID 203
#endif
...
/*
   SensorOpenHABConfiguration: allow remote configuration of some node functions and also send the values out
*/
class SensorOpenHABConfiguration: public Sensor {
  public:
    SensorOpenHABConfiguration(NodeManager& nodeManager);
    // define what to do at each stage of the sketch
    void onBefore();
    void onSetup();
    void onLoop(Child* child);
