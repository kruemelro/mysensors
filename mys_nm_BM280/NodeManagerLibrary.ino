...
SensorBME280::SensorBME280(NodeManager& node_manager, int child_id): SensorBosch(node_manager, child_id) {
  ...
  new ChildFloat(this,_node->getAvailableChildId(child_id),S_TEMP,V_TEMP,"Temperature");
  new ChildFloat(this,_node->getAvailableChildId(child_id+1),S_HUM,V_HUM,"Humidity");
  new ChildFloat(this,_node->getAvailableChildId(child_id+2),S_BARO,V_PRESSURE,"Pressure");
  new ChildString(this,_node->getAvailableChildId(child_id+3),S_BARO,V_FORECAST,"Forecast");
}
...
void SensorConfiguration::onBefore() {
  new ChildInt(this,CONFIGURATION_CHILD_ID,S_CUSTOM,"RemoteAPI");
}
...
/*
   SensorOpenHABConfiguration
*/
#ifdef USE_OPENHABCONFIGURATION

// contructor
SensorOpenHABConfiguration::SensorOpenHABConfiguration(NodeManager& node_manager): Sensor(node_manager) {
  _name = "OPENHABCONFIG";
  setReportIntervalMinutes(60);
}

// what to do during before
void SensorOpenHABConfiguration::onBefore() {
  new ChildInt(this,OPENHABCONFIGURATION_CHILD_ID,S_CUSTOM,"openHAB");
}

// what to do during setup
void SensorOpenHABConfiguration::onSetup() {

}

// what to do during loop
void SensorOpenHABConfiguration::onLoop(Child* child) {
  int16_t value = _node->getSleepSeconds();
  value = value / 60;
  _node->sendMessage(OPENHABCONFIGURATION_CHILD_ID,V_VAR1,value);
}
